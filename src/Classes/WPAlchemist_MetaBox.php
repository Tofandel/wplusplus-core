...
class WP_Alchemist_MetaBox {
	...
	protected function fix_serialized_string_type( $serialized_data ) {
		return preg_replace_callback( '!s:(\d+):"(.*?)";!s', array(
			$this,
			'fix_serialized_string_type_callback'
		), stripslashes( $serialized_data ) );
	}
	 ...
	 * @return bool
	 */
	public function remove_filter( $tag, $function_to_remove, $accepted_args = 1 ) {
		$tag = $this->_get_filter_tag( $tag );

		return remove_filter( $tag, $function_to_remove, $accepted_args );
	}
	 ...
	 * @return bool
	 */
	public function remove_action( $tag, $function_to_remove, $accepted_args = 1 ) {
		$tag = $this->_get_action_tag( $tag );

		return remove_action( $tag, $function_to_remove, $accepted_args );
	}
	...
	public function can_output() {
		...
		(
			...
			if ( ! empty( $this->exclude_template ) && is_array( $this->exclude_template ) ) {
