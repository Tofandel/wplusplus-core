...
if ( ! class_exists( 'ReduxFrameworkPlugin', false ) ) {
	...
	class ReduxFrameworkPlugin {
		...
		public function includes() {
			...
			if ( isset( ReduxCore::$_as_plugin ) ) {
				ReduxCore::$_as_plugin = true;
			}
		...
		public function load_first() {
			...
			if ( $plugins ) {
				if ( array_search( $path, $plugins, true ) === $key ) {
				...
				if ( class_exists( 'Redux_Pro' ) ) {
					...
					$key = array_search( $path . '/redux-pro.php', $plugins, true );
					if ( false !== $key ) {
						array_splice( $plugins, $key, 1 );
		...
		public function options_toggle_check() {
			...
			if ( 'plugins.php' === $pagenow && is_admin() && isset( $_GET['ReduxFrameworkPlugin'] ) && isset( $_GET['nonce'] ) && wp_verify_nonce( sanitize_key( $_GET['nonce'] ), 'redux_framework_demo' ) ) {
				$url = './plugins.php';

				if ( 'demo' === $_GET['ReduxFrameworkPlugin'] ) {
		...
		public function plugin_metalinks( $links, $file ) {
			if ( strpos( $file, 'redux-framework.php' ) !== false && is_plugin_active( $file ) ) {
				...
				$new_links = array(
					'<a href="//docs.reduxframework.com/" target="_blank">' . esc_html__( 'Docs', 'redux-framework' ) . '</a>',
					'<a href="//github.com/ReduxFramework/redux-framework" target="_blank">' . esc_html__( 'Repo', 'redux-framework' ) . '</a>',
					'<a href="//build.reduxframework.com/" target="_blank">' . esc_html__( 'Builder', 'redux-framework' ) . '</a>',
				...
				if ( ( is_multisite() && $this->plugin_network_activated ) || ! is_network_admin() || ! is_multisite() ) {
					...
					if ( $this->options['demo'] ) {
						$new_links[3] .= '<br /><span style="display: block; padding-top: 6px;"><a href="./plugins.php?ReduxFrameworkPlugin=demo&nonce=' . $nonce . '" style="color: #bc0b0b;">' . esc_html__( 'Deactivate Demo Mode', 'redux-framework' ) . '</a></span>';
					} else {
						$new_links[3] .= '<br /><span style="display: block; padding-top: 6px;"><a href="./plugins.php?ReduxFrameworkPlugin=demo&nonce=' . $nonce . '" style="color: #bc0b0b;">' . esc_html__( 'Activate Demo Mode', 'redux-framework' ) . '</a></span>';
					}
				}
