...
// Don't duplicate me!
if ( ! class_exists( 'ReduxFramework_Section', false ) ) {
	 ...
	 * @since       1.0.0
	 */
	class ReduxFramework_Section extends Redux_Field {
		...
		public function render() {
			...
			<script type="text/javascript">
				jQuery( document ).ready(
					function() {
						jQuery( '#<?php echo esc_attr( $this->field['id'] ); ?>-marker' ).parents( 'tr:first' ).css(
							{display: 'none'} ).prev( 'tr' ).css( 'border-bottom', 'none' );

						const group = jQuery( '#<?php echo esc_attr( $this->field['id'] ); ?>-marker' ).parents(
							'.redux-group-tab:first' );
