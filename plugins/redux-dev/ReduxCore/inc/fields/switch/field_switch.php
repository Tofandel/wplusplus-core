...
if (!class_exists('ReduxFramework_switch', false)) {
    ...
    class ReduxFramework_switch extends Redux_Field {
        ...
        function enqueue() {
            wp_enqueue_script(
                    'redux-field-switch-js', ReduxCore::$_url . 'inc/fields/switch/field_switch' . Redux_Functions::isMin() . '.js', array('jquery', 'redux-js'), $this->timestamp, true
            );

            if ($this->parent->args['dev_mode']) {
                wp_enqueue_style(
                        'redux-field-switch-css', ReduxCore::$_url . 'inc/fields/switch/field_switch.css', array(), $this->timestamp, 'all'
                );
            }
        }
