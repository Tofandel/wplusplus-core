...
if (!class_exists('ReduxFramework_sortable', false)) {

    class ReduxFramework_sortable extends Redux_Field {
        ...
        function render() {
            ...
            foreach ($this->value as $k => $nicename) {
                ...
                if ($this->field['mode'] == "checkbox") {
                    ...
                    echo '<input type="hidden" name="' . esc_attr($this->field['name'] . $this->field['name_suffix']) . '[' . esc_attr($k) . ']' . '" id="' . esc_attr($this->field['id'] . '-' . $k) . '-hidden" value="' . esc_attr($value_display) . '" />';
                ...
                if ($this->field['mode'] != "checkbox") {
                    ...
                    echo '<input rel="' . esc_attr($this->field['id'] . '-' . $k) . '-hidden" class="' . esc_attr($class) . '" ' . $checked . 'type="' . esc_attr($this->field['mode']) . '" ' . esc_attr($name) . 'id="' . esc_attr($this->field['id'] . '[' . $k) . ']" value="' . esc_attr($value_display) . '" placeholder="' . esc_attr($nicename) . '" />';
                }
        ...
        function enqueue() {
            if ($this->parent->args['dev_mode']) {
                wp_enqueue_style(
                        'redux-field-sortable-css', ReduxCore::$_url . 'inc/fields/sortable/field_sortable.css', array(), $this->timestamp, 'all'
                );
            }

            wp_enqueue_script(
                    'redux-field-sortable-js', ReduxCore::$_url . 'inc/fields/sortable/field_sortable' . Redux_Functions::isMin() . '.js', array('jquery', 'redux-js', 'jquery-ui-sortable'), $this->timestamp, true
            );
        }
