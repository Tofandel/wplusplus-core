	...
	class ReduxFramework_extension_customizer {
		...
		function enqueue_controls_css() {
			...
			wp_enqueue_style(
				...
				$this->_extension_url . 'extension_customizer.css',
				array(),
				self::$version,
				'all'
			);
		...
		protected static function get_post_values() {
			if ( empty( self::$post_values ) && isset( $_POST['customized'] ) && ! empty( $_POST['customized'] ) ) { // WPCS: CSRF ok.
				self::$post_values = json_decode( stripslashes_deep( $_POST['customized'] ), true ); // WPCS: CSRF ok.
			}
		}
		...
		public function render( $control ) {
			...
			echo '<th scope="row">' . esc_html( $this->parent->field_head[ $field['id'] ] ) . '</th>';
			echo '<td>';
		...
		public function customizer_save_after( $wp_customize ) {
			...
			$options  = json_decode( stripslashes_deep( $_POST['customized'] ), true ); // WPCS: CSRF ok.
			$compiler = false;
			$changed  = false;
