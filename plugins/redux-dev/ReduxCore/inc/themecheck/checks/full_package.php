...
class Redux_Full_Package implements themecheck {
	...
	function check( $php_files, $css_files, $other_files ) {
		...
		if ( $redux ) {
			...
			if ( ! empty( $errors ) ) {
				$error = '<span class="tc-lead tc-required">REQUIRED</span> ' . esc_html__( 'It appears that you have embedded the full Redux package inside your theme. You need only embed the', 'redux-framework' ) . ' <strong>ReduxCore</strong> ' . esc_html__( 'folder. Embedding anything else will get your rejected from theme submission. Suspected Redux package file(s):', 'redux-framework' );
				$error .= '<ol>';
