...
if ( ! class_exists( 'Redux_ThemeCheck', false) ) {
	...
	class Redux_ThemeCheck {
		...
		public static function get_redux_instance() {
			...
			// If the single instance hasn't been set, set it now.
			if ( null == self::$redux && ReduxCore::$_as_plugin ) {
		...
		public function enqueue_admin_styles() {
			...
			if ( 'appearance_page_themecheck' == $screen->id ) {
				wp_enqueue_style( $this->slug . '-admin-styles', ReduxCore::$_url . 'inc/themecheck/css/admin.css', array(), $this->version );
			}
		}
		...
		public function enqueue_admin_scripts() {
			...
			if ( 'appearance_page_themecheck' == $screen->id ) {
				wp_enqueue_script( $this->slug . '-admin-script', ReduxCore::$_url . 'inc/themecheck/js/admin.js', array( 'jquery' ), $this->version );
