...
<div class="wrap about-wrap redux-status" style="max-width:initial;margin:10px 20px 0 2px;">
	...
	<div class="about-text">
		<?php esc_html_e( 'Our core mantra at Redux is backward compatibility. With hundreds of thousands of instances worldwide, you can be assured that we will take care of you and your clients.', 'redux-framework' ); ?></div>
					...
					<tr>
						...
						if ( $found_files ) {
							foreach ( $found_files as $plugin_name => $found_plugin_files ) {
								?>
								<td>
									<?php echo implode( ', <br/>', esc_html( $found_plugin_files ) ); ?>
								</td>
								<?php
							}
						} else {
					...
					<tr>
						...
						<td>
							...
							foreach ( $ext as $name => $arr ) {
								...
								echo '<a href="http://reduxframework.com/extensions/' . esc_html( str_replace( array( '_' ), '-', $name ) ) . '" target="blank">' . esc_html( ucwords( str_replace( array( '_', '-' ), ' ', $name ) ) ) . '</a> - ' . esc_html( $ver );
								?>
								<br/>
								<?php
							}
		...
		<tr>
			...
			<td>
				...
				if ( is_child_theme() ) {
					...
					echo '<span class="dashicons dashicons-no-alt"></span> &ndash; ' . wp_kses_post( sprintf( __( 'If you are modifying Redux Framework on a parent theme that you did not build personally we recommend using a child theme. See: <a href="%s" target="_blank">How to create a child theme</a>', 'redux-framework' ), 'https://codex.wordpress.org/Child_Themes' ) );
				}
				?>
			</td>
		</tr>
