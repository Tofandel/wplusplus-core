...
if (!class_exists('Redux_Enqueue', false)) {
    ...
    class Redux_Enqueue extends Redux_Class {
        ...
        private function register_styles($core) {
            ...
            wp_register_style(
                    'redux-admin-theme-css', $css, array(), $this->timestamp, 'all'
            );

            wp_enqueue_style(
                    'redux-admin-css', ReduxCore::$_url . "assets/css/redux-admin{$this->min}.css", array('redux-admin-theme-css'), $this->timestamp, 'all'
            );
            ...
            if (!$core->args['dev_mode']) {
                wp_enqueue_style(
                        'redux-fields-css', ReduxCore::$_url . 'assets/css/redux-fields.min.css', array(), $this->timestamp, 'all'
                );
            }
            ...
            //*****************************************************************
            wp_enqueue_style(
                    'select2-css', ReduxCore::$_url . 'assets/css/vendor/select2.min.css', array(), '4.0.4', 'all'
            );
            ...
            wp_register_style(
                    'redux-spectrum-css', ReduxCore::$_url . "assets/css/vendor/spectrum{$this->min}.css", array(), '1.3.3', 'all'
            );
            ...
            //*****************************************************************
            wp_enqueue_style(
                    'redux-elusive-icon', ReduxCore::$_url . "assets/css/vendor/elusive-icons{$this->min}.css", array(), $this->timestamp, 'all'
            );
            ...
            wp_enqueue_style(
                    'qtip-css', ReduxCore::$_url . "assets/css/vendor/qtip{$this->min}.css", array(), '2.2.0', 'all'
            );
            ...
            //*****************************************************************
            wp_enqueue_style(
                    'jquery-ui-css', apply_filters("redux/page/{$core->args['opt_name']}/enqueue/jquery-ui-css", ReduxCore::$_url . 'assets/css/vendor/jquery-ui-1.10.0.custom.css'), array(), $this->timestamp, 'all'
            );
...
//                    );
                ...
                //*****************************************************************
                wp_enqueue_style(
                        'redux-field-media-css', ReduxCore::$_url . 'assets/css/media.css', array(), $this->timestamp, 'all'
                );
            }
            ...
            if (is_rtl()) {
                wp_enqueue_style(
                        'redux-rtl-css', ReduxCore::$_url . 'assets/css/rtl.css', array('redux-admin-css'), $this->timestamp, 'all'
                );
            }
        }
        ...
        private function register_scripts($core) {
            ...
            //*****************************************************************
            wp_register_script(
                    'redux-select2-sortable-js', ReduxCore::$_url . 'assets/js/vendor/select2-sortable/redux.select2.sortable' . $this->min . '.js', array('jquery', 'jquery-ui-sortable'), $this->timestamp, true
            );

            wp_enqueue_script(
                    'select2-js', ReduxCore::$_url . 'assets/js/vendor/select2/select2' . $this->min . '.js`', array('jquery', 'redux-select2-sortable-js'), '4.0.4', true
            );
            ...
            //*****************************************************************
            wp_enqueue_script(
                    'qtip-js', ReduxCore::$_url . 'assets/js/vendor/qtip/qtip' . $this->min . '.js', array('jquery'), '2.2.0', true
            );
            ...
            wp_register_script(
                    'redux-spectrum-js', ReduxCore::$_url . 'assets/js/vendor/spectrum/' . $js_file, array('jquery'), '1.3.3', true
            );
            ...
            //*****************************************************************
            wp_register_script(
                    'redux-vendor', ReduxCore::$_url . 'assets/js/redux-vendors' . $this->min . '.js', array('jquery'), $this->timestamp, true
            );
            ...
            //*****************************************************************
            wp_register_script(
                    'redux-js', ReduxCore::$_url . 'assets/js/redux' . $this->min . '.js', $depArray, $this->timestamp, true
            );

            if ($core->args['async_typography']) {
                wp_enqueue_script(
                        'webfontloader', 'https://' . 'ajax' . '.googleapis' . '.com/ajax/libs/webfont/1.6.26/webfont.js', array('jquery'), '1.6.26', true
                );
            }
        }
                ...
                if ($class_file) {
                    ...
                    if (( method_exists($field_class, 'enqueue') ) || method_exists($field_class, 'localize')) {
                        ...
                        if (method_exists($field_class, 'localize')) {
                            ...
                            if (is_array($shims) && in_array($field['type'], $shims)) {
        ...
        private function set_localized_data($core) {
            ...
            wp_localize_script(
                    'redux-js', 'redux', $this->repeater_data
            );

            wp_localize_script(
                    'redux-js', 'redux_' . $core->args['opt_name'], $core->localize_data
            );
