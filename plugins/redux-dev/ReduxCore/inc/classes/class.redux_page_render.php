	...
	class Redux_Page_Render extends Redux_Class {
		...
		public function load_page() {
			...
			if ( true === $this->show_hints ) {
				...
				// Make URL.
				$url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . esc_attr( $cur_page ) . '&amp;tab=' . esc_attr( $cur_tab ) . '">' . $s . ' hints</a>';
		 ...
		 * @return      void
		 */
		public function field_input( $field, $v = null ) {
			$core = $this->core();
			...
			if ( isset( $field['type'] ) ) {
				...
				if ( class_exists( $field_class ) ) {
					...
					//$core->fields_values[ $field['id'] ] = ( isset( $value['url'] ) && is_array( $value ) ) ? $value['url'] : $value;
