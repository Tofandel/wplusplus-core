        ...
        public function enqueue() {
            ...
            if (!empty($core->typography) && !empty($core->typography) && filter_var($core->args['output'], FILTER_VALIDATE_BOOLEAN)) {
                ...
                if ($core->args['async_typography'] && !empty($core->typography)) {
                    ...
                    wp_enqueue_style(
                            'redux-google-fonts-' . $core->args['opt_name'], $protocol . $typography->make_google_web_font_link($core->typography), array(), $version, 'all'
                    );
                }
            }
        }
            ...
            if (!empty($field['required'])) {
                if (isset($field['required'][0])) {
                    if (!is_array($field['required'][0]) && count($field['required']) == 3) {
                        $parentValue = isset($GLOBALS[$core->args['global_variable']][$field['required'][0]]) ? $GLOBALS[$core->args['global_variable']][$field['required'][0]] : '';
