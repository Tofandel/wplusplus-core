...
if ( ! class_exists( 'Redux_Options', false ) ) {
	...
	class Redux_Options extends Redux_Class {
		...
		public function get() {
			...
			elseif ( $core->args[ 'database' ] === "theme_mods" ) {
				$result = get_theme_mod( $core->args[ 'opt_name' ] . '-mods' );
			}
			elseif ( $core->args[ 'database' ] === 'theme_mods_expanded' ) {
				$result = get_theme_mods();
			}
			elseif ( $core->args[ 'database' ] === 'network' ) {
			...
			//If we switch language in wpml the post/page selected will be in the wrong language
			...
			foreach ( $core->sections as $section ) {
				if ( ! empty( $section[ 'id' ] ) && ! empty( $section[ 'data' ] ) && ! empty( $core->options[ $section[ 'id' ] ] ) && is_int( $core->options[ $section[ 'id' ] ] ) ) {
					$core->options[ $section[ 'id' ] ] = apply_filters( 'wpml_object_id', $core->options[ $section[ 'id' ] ], $section[ 'data' ], true );
				}
			}
			/**
						...
						if ( $field[ 'type' ] == "media" ) {
							if ( isset ( $core->options[ $field[ 'id' ] ][ 'id' ] ) && isset ( $core->options[ $field[ 'id' ] ][ 'url' ] ) && ! empty ( $core->options[ $field[ 'id' ] ][ 'url' ] ) && strpos( $core->options[ $field[ 'id' ] ][ 'url' ], str_replace( 'http://', '', WP_CONTENT_URL ) ) === false ) {
								...
								if ( isset ( $data ) && ! empty ( $data ) ) {
									...
									$data                                           = wp_get_attachment_image_src( $core->options[ $field[ 'id' ] ][ 'id' ], array(
										150,
										150
									) );
						...
						if ( $field[ 'type' ] == "background" ) {
							if ( isset ( $core->options[ $field[ 'id' ] ][ 'media' ][ 'id' ] ) && isset ( $core->options[ $field[ 'id' ] ][ 'background-image' ] ) && ! empty ( $core->options[ $field[ 'id' ] ][ 'background-image' ] ) && strpos( $core->options[ $field[ 'id' ] ][ 'background-image' ], str_replace( 'http://', '', WP_CONTENT_URL ) ) === false ) {
								...
								if ( isset ( $data ) && ! empty ( $data ) ) {
									...
									$data                                                      = wp_get_attachment_image_src( $core->options[ $field[ 'id' ] ][ 'media' ][ 'id' ], array(
										150,
										150
									) );
