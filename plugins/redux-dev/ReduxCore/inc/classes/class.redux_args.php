...
if ( ! class_exists( 'Redux_Args', false ) ) {
	...
	class Redux_Args {
		...
		private function args( $args ) {
			...
			// TODO:  Get live_preview nonce here
		...
		private function no_errors_please( $args ) {
			...
			if ( empty( $args['footer_credit'] ) ) {
				...
				$args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( esc_html__( 'Powered by  %1$s', 'redux-framework' ), '<a href="' . esc_url( '//www.reduxframework.com' ) . '" target="_blank">' . esc_html__( 'Redux Framework', 'redux-framework' ) . '</a> v' . ReduxCore::$_version ) . '</span>';
			}
		...
		private function change_demo_defaults( $args ) {
			if ( $args['dev_mode'] || true === Redux_Helpers::isLocalHost() ) {
