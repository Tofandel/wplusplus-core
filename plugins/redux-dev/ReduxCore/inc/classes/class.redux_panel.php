...
if ( ! class_exists( 'Redux_Panel', false ) ) {
	...
	class Redux_Panel {
		...
		public function __construct( $parent ) {
			$this->parent        = $parent;
			$this->template_path = ReduxCore::$_dir . 'templates/panel/';
			$this->original_path = ReduxCore::$_dir . 'templates/panel/';
		...
		private function panel_template() {
			...
			// Security is vital!
			echo '<input type="hidden" id="ajaxsecurity" name="security" value="' . esc_attr( wp_create_nonce( 'redux_ajax_nonce' . $this->parent->args['opt_name'] ) ) . '" />';
				...
				if ( $developer_theme_file ) {
					...
					if ( $core_version && $developer_version && version_compare( $developer_version, $core_version, '<' ) ) {
						...
						<div id="message" class="error redux-message">
							<p>
								<strong><?php esc_html_e( 'Your panel has bundled outdated copies of Redux Framework template files', 'redux-framework' ) . '</strong>&#8211;' . esc_html_e( 'If you encounter functionality issues this could be the reason. Ensure you update or remove them.', 'redux-framework' ); ?>
							</p>
						</div>
						<?php
