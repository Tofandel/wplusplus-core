...
if ( ! class_exists( 'Redux_Helpers', false ) ) {
	...
	class Redux_Helpers {
		...
		public static function isLocalHost() {
			return ( isset( ReduxCore::$_server[ 'REMOTE_ADDR' ] ) && ( ReduxCore::$_server[ 'REMOTE_ADDR' ] === '127.0.0.1' || ReduxCore::$_server[ 'REMOTE_ADDR' ] === 'localhost' || ReduxCore::$_server[ 'SERVER_ADDR' ] === "::1" ) ) ? 1 : 0;
		}
		...
		public static function getTrackingObject() {
			...
			$data = array(
				'_id'       => $hash,
				'localhost' => ( ReduxCore::$_server[ 'REMOTE_ADDR' ] === '127.0.0.1' ) ? 1 : 0,
				'php'       => $version,
				'site'      => array(
					'hash'      => $hash,
				...
				'comments'  => array(
					...
					'pings'    => $comments_query->query( array(
						'count' => true,
						'type'  => 'pingback'
					) ),
				),
		...
		public static function trackingObject() {
			...
			$data = wp_remote_post( 'http://verify.redux.io', array(
				'body' => array(
					'hash' => $_GET[ 'action' ],
		...
		public static function isParentTheme( $file ) {
			$file = Redux_Helpers::wp_normalize_path( $file );
			$dir  = Redux_Helpers::wp_normalize_path( get_template_directory() );
		...
		public static function isChildTheme( $file ) {
			$file = Redux_Helpers::wp_normalize_path( $file );
			$dir  = Redux_Helpers::wp_normalize_path( get_stylesheet_directory() );
		...
		public static function get_hash() {
			return md5( network_site_url() . '-' . ReduxCore::$_server[ 'REMOTE_ADDR' ] );
		}
		...
		public static function get_wp_themes() {
			...
			foreach ( $themes as $theme ) {
				$path          = Redux_Helpers::wp_normalize_path( trailingslashit( $theme->get_theme_root() ) . $theme->get_template() );
				$theme_paths[] = $path;
				if ( $path != Redux_Helpers::wp_normalize_path( realpath( $path ) ) ) {
					$theme_paths[] = Redux_Helpers::wp_normalize_path( realpath( $path ) );
				}
				$wp_theme_paths[ $path ] = Redux_Helpers::wp_normalize_path( realpath( $path ) );
			}

			return array( 'full_paths' => $wp_theme_paths, 'theme_paths' => $theme );
		}
		...
		public static function is_inside_plugin( $file ) {
			...
			if ( $file != "/" . $plugin_basename ) {
				...
				return array(
					...
					'path'     => Redux_Helpers::wp_normalize_path( $file ),
					'url'      => plugins_url( $plugin_basename ),
					'realpath' => Redux_Helpers::wp_normalize_path( dirname( realpath( $file ) ) )
				);
			}
		...
		public static function compileSystemStatus( $json_output = false, $remote_checks = false ) {
			...
			$sysinfo[ 'browser' ] = array(
				...
				'platform' => $browser->getPlatform(),//'mobile'   => $browser->isMobile() ? 'true' : 'false',
			);

			$sysinfo[ 'server_info' ] = esc_html( ReduxCore::$_server[ 'SERVER_SOFTWARE' ] );
		...
		public static function get_extension_dir( $dir ) {
			return trailingslashit( Redux_Helpers::wp_normalize_path( dirname( $dir ) ) );
		}

		public static function get_extension_url( $dir ) {
			$ext_dir = Redux_Helpers::get_extension_dir( $dir );
			$ext_url = str_replace( Redux_Helpers::wp_normalize_path( WP_CONTENT_DIR ), WP_CONTENT_URL, $ext_dir );
