...
(function( $ ) {
    ...
    $.redux.ajax_save = function( button ) {
        ...
        if ( redux.optName.fields.hasOwnProperty( "editor" ) ) {
            $.each(
                redux.optName.fields.editor, function( $key, $index ) {
        ...
        $.ajax(
            {
                ...
                success: function( response ) {
                    ...
                    if ( response.action && response.action === "reload" ) {
                        ...
                        //redux.optName.defaults = response.defaults;
...
function redux_change( variable ) {
	(function( $ ) {
		...
		var th = rContainer.find( '.redux-group-tab-link-a[data-key="' + id + '"]' ).parents(
			'.redux-group-tab-link-li:first' );
