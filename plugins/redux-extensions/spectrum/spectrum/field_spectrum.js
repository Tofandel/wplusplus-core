(function ($) {
	...
	redux.field_objects.spectrum = redux.field_objects.spectrum || {};
	redux.field_objects.spectrum.fieldID = '';

	redux.field_objects.spectrum.hexToRGBA = function (hex, alpha) {
		if (hex === null) {
			result = '';
		} else {
			hex = hex.replace('#', '');
	...
	redux.field_objects.spectrum.init = function (selector) {
		if (!selector) {
			selector = $(document).find(".redux-group-tab:visible").find('.redux-container-spectrum:visible');
		}
		...
		$(selector).each(
			function () {
				...
				redux.field_objects.spectrum.modInit(el);
				redux.field_objects.spectrum.initColorPicker(el);
			});
	};

	redux.field_objects.spectrum.modInit = function (el) {

		redux.field_objects.spectrum.fieldID = el.find('.redux-spectrum-container').data('id');

		// dev_mode
		var dev_mode = Boolean(el.find('.redux-spectrum-container').data('dev-mode'));

		// Add tag to footer, dev mode only.
		if (dev_mode === true) {
			var ver = $('.redux-spectrum-container').data('version');
			var dev_html = $('div.redux-timer').html();

			if (dev_html !== undefined) {
				var pos = dev_html.indexOf('Spectrum Color Picker');

				if (pos === -1) {
					$('div.redux-timer').html(dev_html + '<br/>Spectrum Color Picker extension v.' + ver);
				}
			}
		}
	};
	...
	redux.field_objects.spectrum.initColorPicker = function (el) {

		// Get field ID
		var field_id = redux.field_objects.spectrum.fieldID;

		// Get the color scheme container
		var colorpickerInput = el.find('.redux-spectrum');
		...
		var container = el.find('.redux-spectrum-container');
		...
		colorpickerInput.spectrum({
			...
			change: function (color) {
				...
				var rgbaVal = redux.field_objects.spectrum.hexToRGBA(colorVal, alphaVal);
				...
				redux_change(el.find('.redux-spectrum-container'));
			}
		});
	};
})(jQuery);