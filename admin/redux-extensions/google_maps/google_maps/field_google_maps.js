...
(function ($) {
	...
	var g_map, g_marker, g_autoComplete, g_LatLng;
	...
	redux.field_objects.google_maps.init = function (selector) {
		...
		$(selector).each(
			function (i) {
				...
				var delayRender = Boolean(el.find('.redux_framework_google_maps').data('delay-render'));
				...
				// fucking radio button won't check on it's own, for some reason.
				setTimeout(function () {
					$("#changetype-all").attr('checked', 'checked');
				}, 1);
			}
		);
	};
	...
	redux.field_objects.google_maps.clickHandler = function (el) {
		...
		el.find('.google_m_api_key_button').click(
			function (e) {
				...
				if (wrapper.is(':visible')) {
					...
					wrapper.slideUp(
						'fast', function () {
							el.find('#google_m_api_key_input').focus();
						}
					);
				} else {
		...
		el.find('.google_m_autocomplete').click(
			function (e) {
				this.focus();
				this.select();
				e.preventDefault();
	...
	redux.field_objects.google_maps.renderMap = function (el, mapClass) {
		...
		// data bindings
		var scrollWheel = Boolean(mapClass.data('scroll-wheel'));
		var streetView = Boolean(mapClass.data('street-view'));
		var mapType = Boolean(mapClass.data('map-type'));

		var address = mapClass.data('address');
		address = decodeURIComponent(address);
		address = address.trim();

		// set default Lat/lng
		var defLat = $('#' + canvas).data('default-lat');
		var defLong = $('#' + canvas).data('default-long');
		var defaultZoom = $('#' + canvas).data('default-zoom');

		// Eval whether or not to set maps based on lat/lng or address
		var noLatLng = false;
		if (address !== '') {
			if (defLat === '' || defLong === '') {
				noLatLng = true;
			...
			geocoder.geocode(
				...
				function (results, status) {
					...
					if (status === google.maps.GeocoderStatus.OK) {
						...
						var latitude = el.find('#' + containerID + "_latitude");
						latitude.val(results[0].geometry.location.lat());

						var longitude = el.find('#' + containerID + "_longitude");
	...
	redux.field_objects.google_maps.initMap = function (el, idx, containerID, delayRender) {
		...
		// from another scrip, add the 'render' class so rerendering
		// does not occur.  it messes things up.
		var delayed = Boolean(mapClass.data('delay-render'));
		if (delayed === true) {
	...
	redux.field_objects.google_maps.renderControls = function (el, autoComplete, mapClass) {
		...
		// Bind ojects to the map.
		g_autoComplete = new google.maps.places.Autocomplete(input);
		g_autoComplete.bindTo('bounds', g_map);

		// Get the marker tooltip data
		var markerTooltip = mapClass.data('marker-tooltip');
	...
	redux.field_objects.google_maps.addListeners = function (el, mapClass, marker, infoWindow) {
		...
		google.maps.event.addListener(
			g_autoComplete, 'place_changed', function () {
				infoWindow.close();
		...
		redux.field_objects.google_maps.setupClickListener('changetype-all', []);
		redux.field_objects.google_maps.setupClickListener('changetype-address', ['address']);
		redux.field_objects.google_maps.setupClickListener('changetype-establishment', ['establishment']);
		redux.field_objects.google_maps.setupClickListener('changetype-geocode', ['geocode']);

		// Marker drag
		google.maps.event.addListener(
			marker, 'drag', function (event) {
	...
	redux.field_objects.google_maps.fillInAddress = function (el, latitude, longitude) {
		...
		// gotta do it this way to check for the addres!
		...
		for (var component in componentForm) {
			...
			var component = containerID + "_" + component;
		...
		for (var i = 0; i < place.address_components.length; i++) {
			...
			if (componentForm[addressType]) {
				// push in the dynamic form element ID again
				var _d_addressType = containerID + "_" + addressType;
